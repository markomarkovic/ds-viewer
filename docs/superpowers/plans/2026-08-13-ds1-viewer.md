...
export type _keep = [typeof bad1, typeof bad2, typeof bad3, typeof bad4, typeof s]
```
...
export function parseDs1(buf: ArrayBuffer, fileName: string): ParseOutcome {
  ...
  for (let i = 0; i < n; i += 4) {
    ...
    if ((b0 & 0x80) === 0) {
      ...
      if (b0 !== 0 || bytes[i + 1]! !== 0 || bytes[i + 2]! !== 0 || bytes[i + 3]! !== 0)
        partial = true
      continue
    }
    ...
    if (type === 0) {
      if (sub === 0) {
        ...
        cur.start = new Date(2000 + onDate[0], onDate[1] - 1, onDate[2], b1, b2, b3)
...
test('lowpass matches ds1.py bit-for-bit', () => {
  const g50 = [
    133.92776489257812, 167.85552978515625, 185.7110595703125,
    221.422119140625, 217.84423828125, 198.1884765625, 187.626953125,
    160.87890625, 169.5703125, 183.046875,
  ]
  const g20 = [
...
test('percentileDeci property: matches naive sort on random data', () => {
  ...
  for (let trial = 0; trial < 20; trial++) {
    ...
    const data = Uint16Array.from({ length: n }, () =>
      Math.floor(rnd() * 250)
    )
    const hist = new Uint32Array(301)
...
test('buildNight computes spec metrics on a synthetic night', () => {
  const buf = simpleNight({ sampleCount: 3000, pressDeci: 64, apneaAt: [100, 200] })
  const { sessions, partial } = parseDs1(buf, '13082026.ds1')
...
export function percentileDeci(
  hist: Uint32Array,
  n: number,
  p: number
): Deci {
...
export type Action =
  | { type: 'ingest-started'; accepted: number; skippedCount: number; refused: string[] }
...
const night = (name: string, y: number, m: number, d: number): Night => ({
  ...
  hours: 7,
  samples: 252000,
  press: { avg: deci(61), median: deci(62), p90: deci(66), p95: deci(69), max: deci(75) },
...
test('night-loaded inserts sorted by date descending', () => {
  ...
  s = reducer(s, { type: 'night-loaded', night: night('13082026', 2026, 8, 13) })
...
test('visibleNights filters by range inclusive', () => {
  ...
  expect(visibleNights(s).map((n) => n.name)).toEqual([
    '13082026',
    '05082026',
  ])
})
  ...
  switch (action.type) {
    case 'ingest-started': {
      ...
      if (action.skippedCount)
        notices.push(notice('info', `${action.skippedCount} non-.ds1 files skipped`))
      if (action.accepted > 300)
        notices.push(
          notice('info', `${action.accepted} files is a lot; parsing may take a while`)
        )
...
export function App() {
  ...
  useEffect(() => {
    const onDrop = (e: DragEvent) => {
      e.preventDefault()
      setDragOver(false)
      if (e.dataTransfer) void filesFromDataTransfer(e.dataTransfer.items).then(ingest)
    }
  ...
  return (
    <main class="container" style={dragOver ? 'outline: 3px dashed var(--pico-primary)' : ''}>
      <FilePickers onFiles={ingest} pending={state.pending} nights={state.nights.length} />
      {state.notices.map((n) => (
        <article key={n.id} role="alert">
          {n.text}{' '}
          <a href="#" onClick={(e) => { e.preventDefault(); dispatch({ type: 'dismiss-notice', id: n.id }) }}>
            dismiss
          </a>
        </article>
      ))}
      ...
      <ul>
        ...
        <li>
          <label role="button" class="secondary">
            ...
            <input type="file" hidden {...{ webkitdirectory: true }} onChange={pick} />
          </label>
        </li>
      </ul>
    </nav>
    ...
    <table>
      ...
      <tbody>
        {rows.map((n) => {
          ...
          return (
            <tr key={n.name} style="cursor:pointer" onClick={() => onSelect(n.name)}>
              <td>
...
export function RangeSelector({
  ...
  return (
    <section>
      <div role="group">
        <button class="outline" onClick={() => preset(7)}>7d</button>
        <button class="outline" onClick={() => preset(30)}>30d</button>
        <button class="outline" onClick={() => preset(90)}>90d</button>
        <button class="outline" onClick={() => preset(null)}>all</button>
        <input
          type="date"
          value={toInput(range.from)}
          onChange={(e) =>
            onRange({ ...range, from: fromInput((e.currentTarget as HTMLInputElement).value) })
          }
        />
        <input
          type="date"
          ...
          onChange={(e) =>
            onRange({ ...range, to: fromInput((e.currentTarget as HTMLInputElement).value) })
          }
        />
      </div>
      <Chart
  ...
  return (
    <section>
      <nav>
        <ul>
          <li>
            <a href="#" onClick={(e) => { e.preventDefault(); onBack() }}>← all nights</a>
          </li>
          <li>
        ...
        <ul>
          {(['pressure', 'leak', 'events'] as const).map((t) => (
            <li key={t}>
              <button class={tab === t ? '' : 'outline'} onClick={() => setTab(t)}>
                {t}
              </button>
            </li>
          ))}
...
function EventLanes({
  ...
  return (
    <div>
      {lanes.map((lane) => (
        <div key={lane.kind} style="display:flex; align-items:center; gap:.5rem">
  ...
  return (
    <section>
      <div role="group">
        ...
        <button class="outline" onClick={() => setView({ startSec: firstStart, windowSec: 300 })}>
          Home
        </button>
        <button class="outline" onClick={() => page(-1)}>◀ Page</button>
        <button class="outline" onClick={() => page(1)}>Page ▶</button>
        <button
          class="outline"
          onClick={() =>
            setView((v) => ({ ...v, startSec: clampStart(lastEnd - v.windowSec, v.windowSec) }))
          }
        >
          End
        </button>
        <small>
          {clockLabel(view.startSec)} – {clockLabel(view.startSec + view.windowSec)}
        </small>
      </div>
      <Chart
        deps={[night.name, view.startSec, view.windowSec]}
        build={(el, width) => {
          const p = windowData(night, 'press', view.startSec, view.windowSec, width)
          const f = windowData(night, 'flow', view.startSec, view.windowSec, width)
          return new uPlot(
            {
              width,
              height: 320,
              scales: {
                x: { time: false, range: [view.startSec, view.startSec + view.windowSec] },
                ...
                draw: [
                  (u) => {
                    ...
                    for (const sec of apneas) {
                      if (sec < view.startSec || sec > view.startSec + view.windowSec) continue
...
d('differential vs ds1.py', () => {
  ...
  test('waveform CSVs match sample-for-sample', () => {
    ...
    for (const [stem, night] of nights) {
      const csv = readFileSync(join(out, `${stem}.csv`), 'utf8').trim().split('\n')
      ...
      for (let si = 0; si < night.sessions.length; si++) {
        ...
        for (let i = 0; i < s.press.length; i++, li++) {
          ...
          if (Number(c[0]) !== si + 1) throw new Error(`session misalign ${stem}:${li}`)
...
Add to `README.md` after the *Usage* section (adjusting the Status list to drop "A graphical viewer is *not* implemented yet"):
